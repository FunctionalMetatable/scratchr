 ...
 */
class PaginationSecondaryComponent extends Object
{
 ...
 * @access public
 */
	var $ajaxDivUpdate = "ajax_pagination_secondary";
/**
 ...
 */
    function init($criteria=NULL,$parameters=Array(),$options=Array(), $overload_count = -100)
    {
		...
		if ((isset($this->paging_secondary["show"]))&&(!in_array($this->paging_secondary["show"],$this->resultsPerPage)))
		{
			$this->paging_secondary["show"]=$this->paging_secondary['Defaults']['show'];
		}
		...
		$this->paging_secondary['total'] = $count;
		$this->trimResultsPerPage($count);

		$this->_setPrivateParameter("resultsPerPage");

    	$this->paging_secondary['pageCount'] = ceil($count / $this->paging_secondary['show'] );

    	$this->controller->set('paging_secondary',$this->paging_secondary);
    	
        $this->order = $this->paging_secondary['sortByClass'].".".$this->paging_secondary['sortBy'].' '.strtoupper($this->paging_secondary['direction']);
        
    	// For backwards compatability & clarity
    	$this->limit = $this->paging_secondary['show'];
    	$this->page = $this->paging_secondary['page'];
    	
    	// For less code in the calling method..
    	return (Array($this->order,$this->paging_secondary['show'],$this->paging_secondary['page']));
	}
 ...
 */
    ...
    {
      if ((($this->paging_secondary['page']-1)*$this->paging_secondary['show'])>=$count) 
      {
            $this->paging_secondary['page'] = floor($count/$this->paging_secondary['show']+0.99);
      }
    }
 ...
 */
	...
	{
		$this->paging_secondary['importParams']=$parameters;
		$this->paging_secondary['Defaults'] = Array (
										"page"=>$this->page,
										"show"=>$this->show,
										"sortBy"=>$this->sortBy,
	...
	function _initURL()
	{
		...
		else // No url in the parameters, derive it.
		{
			...
			else
			{
				...
				$this->url .= "/".$action;
				if (isset($this->paging_secondary['importParams']['_unamedParameters']))
				{
					$unnamedString = implode ("/", $this->paging_secondary['importParams']['_unamedParameters']);
					$this->url .= "/".$unnamedString;
					unset($this->paging_secondary['importParams']['_unamedParameters']);
				}
				$this->url .= "/";
			}
		}
	...
	function _check4Form()
	{
		if(isset($this->controller->data['pagination']))
		{
			if (isset($this->controller->data['pagination']['sortByComposite']))
			{
				...
				{
					$this->controller->data['pagination']['sortByClass'] = $this->paging_secondary['Defaults']['sortByClass'];
				}
			...
			foreach($this->controller->data['pagination'] as $parameter=>$value)
			{
				...
				{
					$this->paging_secondary[$parameter] = $this->Sanitize->paranoid($value,array("-","_"));
				}				
			}
		}
	}
 ...
 */
	...
	{
		$this->paging_secondary[$parameter]= $this->$parameter;
	}
 ...
 */
	function _setParameter($parameter,$parameters=Array(),$field=NULL)
	{
		...
		if (in_array($parameter, $this->privateParams))
		{
			$this->paging_secondary[$field] = $this->paging_secondary['Defaults'][$field];
		}
		else
		{
			if ($this->paramStyle=="get")
			{
				...
				{
					$this->paging_secondary[$field] = $this->Sanitize->paranoid($_GET[$parameter],array("-","_"));		
				}
				else
				{
					$this->paging_secondary[$field]= $this->$field;
				}
			}
			elseif ($this->paramStyle=="pretty")
			{
				...
				{
					$this->paging_secondary[$field] = $this->Sanitize->paranoid($parameters[$parameter],array("-","_"));		
				}
				else
				{
					$this->paging_secondary[$field]= $this->$field;
