 ...
 * @access public
 */
	var $ajaxDivUpdate = "ajax_pagination_ternary";
/**
 ...
 */
    function init($criteria=NULL,$parameters=Array(),$options=Array())
    {
		...
		if ((isset($this->paging_ternary["show"]))&&(!in_array($this->paging_ternary["show"],$this->resultsPerPage)))
		{
			$this->paging_ternary["show"]=$this->paging_ternary['Defaults']['show'];
		}
		...
		$this->paging_ternary['total'] = $count;
		$this->trimResultsPerPage($count);

		$this->_setPrivateParameter("resultsPerPage");

    	$this->paging_ternary['pageCount'] = ceil($count / $this->paging_ternary['show'] );

    	$this->controller->set('paging_ternary',$this->paging_ternary);
    	
        $this->order = $this->paging_ternary['sortByClass'].".".$this->paging_ternary['sortBy'].' '.strtoupper($this->paging_ternary['direction']);
        
    	// For backwards compatability & clarity
    	$this->limit = $this->paging_ternary['show'];
    	$this->page = $this->paging_ternary['page'];
    	
    	// For less code in the calling method..
    	return (Array($this->order,$this->paging_ternary['show'],$this->paging_ternary['page']));
	}
 ...
 */
    ...
    {
      if ((($this->paging_ternary['page']-1)*$this->paging_ternary['show'])>=$count) 
      {
            $this->paging_ternary['page'] = floor($count/$this->paging_ternary['show']+0.99);
      }
    }
 ...
 */
	...
	{
		$this->paging_ternary['importParams']=$parameters;
		$this->paging_ternary['Defaults'] = Array (
										"page"=>$this->page,
										"show"=>$this->show,
										"sortBy"=>$this->sortBy,
	...
	function _initURL()
	{
		...
		else // No url in the parameters, derive it.
		{
			...
			else
			{
				...
				$this->url .= "/".$action;
				if (isset($this->paging_ternary['importParams']['_unamedParameters']))
				{
					$unnamedString = implode ("/", $this->paging_ternary['importParams']['_unamedParameters']);
					$this->url .= "/".$unnamedString;
					unset($this->paging_ternary['importParams']['_unamedParameters']);
				}
				$this->url .= "/";
			}
		}
	...
	function _check4Form()
	{
		if(isset($this->controller->data['pagination']))
		{
			if (isset($this->controller->data['pagination']['sortByComposite']))
			{
				...
				{
					$this->controller->data['pagination']['sortByClass'] = $this->paging_ternary['Defaults']['sortByClass'];
				}
			...
			foreach($this->controller->data['pagination'] as $parameter=>$value)
			{
				...
				{
					$this->paging_ternary[$parameter] = $this->Sanitize->paranoid($value,array("-","_"));
				}				
			}
		}
	}
 ...
 */
	...
	{
		$this->paging_ternary[$parameter]= $this->$parameter;
	}
 ...
 */
	function _setParameter($parameter,$parameters=Array(),$field=NULL)
	{
		...
		if (in_array($parameter, $this->privateParams))
		{
			$this->paging_ternary[$field] = $this->paging_ternary['Defaults'][$field];
		}
		else
		{
			if ($this->paramStyle=="get")
			{
				...
				{
					$this->paging_ternary[$field] = $this->Sanitize->paranoid($_GET[$parameter],array("-","_"));		
				}
				else
				{
					$this->paging_ternary[$field]= $this->$field;
				}
			}
			elseif ($this->paramStyle=="pretty")
			{
				...
				{
					$this->paging_ternary[$field] = $this->Sanitize->paranoid($parameters[$parameter],array("-","_"));		
				}
				else
				{
					$this->paging_ternary[$field]= $this->$field;
