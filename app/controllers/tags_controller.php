...
Class TagsController extends AppController {
	...
	$tag =  $this->Tag->find("name = '$tag_name'");
		...
		##############################################################################################
		# If we are sorting by the project creation date we want to speed up the 
		# query by eliminating full table scan. Since creation date is related to id we are 
		# gonna limit to last 500 projects
		if ($option == "creation") {
			$order = 'Project.created DESC';
			$result = $this->Project->query("SELECT MAX(projects.id) AS maxid FROM projects");
			$projectid_min = intval($result[0][0]['maxid']) - 500; 
			$final_criteria = "Project.id >  $projectid_min AND " . $final_criteria;
		}
