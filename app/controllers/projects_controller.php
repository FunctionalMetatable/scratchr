		...
		{	
			...
			if (!isset($binary_file['error']) || $binary_file['error']) 
			{
				$this->setFlash(___("Select a scratch file", true));
					$this->redirect('/projects/share_project');
			...
			if ($this->isempty($pname) || $this->isempty($username)) {
				...
				if($this->ProjectShare->save($record)) {
					...
					if (empty($current_related_project)) {
						...
						{	
							$count=$this->ProjectShare->findCount(array('ProjectShare.user_id'=>$user_shared_id , 'ProjectShare.related_project_id'=>$current_related_project['Project']['id']));
							//$this->log("count:$count");
							if($count ==0)
							{
							$total_remixer = $current_related_project['Project']['remixer'] + 1;
